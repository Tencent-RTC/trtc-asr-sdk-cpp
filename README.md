...
recognizer.SetEnableSpeakerContext(trtc_asr::v3::kSpeakerContextSync); // 1=同步，2=异步（见下表）
recognizer.Start();
if (recognizer.GetSpeakerContinue().has_value()) {   // 也可在 OnRecognitionStart 回调里读
  // 持久化，供下次重连使用
...
recognizer.Start(); // 首响应 continue_status=resumed 表示说话人锚点已恢复
```
...
> `speaker_context_id`，语义与上文一致；首响应同样携带 `speaker_continue`。C++ 侧 API 为
...
- **存量**：v2（`trtc_asr::` 顶层 API）继续全量可用，无需任何改动，文档见 [docs/v2_protocol.md](./docs/v2_protocol.md)。
...
`trtc_asr::` 顶层 API 继续维护，文档见 [docs/v2_protocol.md](./docs/v2_protocol.md)。v2 与 v3 的下行消息结构一致，
listener 用法相同，切换协议版本只需改 include 与构造方式。
