...
- `auth`: identity and signature — `sdkappid` (string), `usersig` (TRTC signature; the identifier is `voice_id` for realtime and `request_id` for HTTP), `request_id` (HTTP, a UUID generated by the SDK).
- `params`: recognition options (engine, VAD, hotwords, filters, …), named like the v2 realtime query parameters.

Realtime: within **3 seconds** of the WebSocket handshake, send one start frame:
...
HTTP: the body is the symmetric `{"auth":{...},"params":{...}}` envelope and the response is flat (no `Response` wrapper):
