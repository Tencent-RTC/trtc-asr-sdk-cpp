...
C++ SDK for Tencent TRTC speech recognition (ASR): realtime recognition (WebSocket), one-shot sentence recognition (HTTP) and asynchronous audio file recognition (HTTP).

This SDK targets the **v3 protocol**: only `SdkAppID` + `SecretKey` are required (no Tencent Cloud AppID), requests use separate `auth` / `params` blocks, everything is snake_case, and responses are flat with numeric codes. The v3 client lives in the `trtc_asr::v3` namespace (`#include "trtc_asr/v3.h"`).

> [English](./README.en.md) | [中文](./README.md)
>
> SDKs: [Go](https://github.com/Tencent-RTC/trtc-asr-sdk-go) | [Python](https://github.com/Tencent-RTC/trtc-asr-sdk-python) | [Node.js](https://github.com/Tencent-RTC/trtc-asr-sdk-nodejs) | [Java](https://github.com/Tencent-RTC/trtc-asr-sdk-java) | [Rust](https://github.com/Tencent-RTC/trtc-asr-sdk-rust)
>
...
`{host}` is `asr.cloud-rtc.com` (China) or `asr-intl.cloud-rtc.com` (international, `credential.set_site(kSiteIntl)`).
...
- The signature is bound to the site: `credential.set_site(kSiteIntl)` selects both the host and the verification cluster — do not mix China-site and international-site credentials.
...
recognizer.SetSpeakerDiarization(trtc_asr::v3::kSpeakerDiarizationCluster); // 1: anonymous clustering
...
recognizer.SetEnableSpeakerContext(trtc_asr::v3::kSpeakerContextSync); // 1=sync, 2=async (see below)
recognizer.Start();
if (recognizer.GetSpeakerContinue().has_value()) {  // also delivered to OnRecognitionStart
  save("speaker_context_id", recognizer.GetSpeakerContinue()->speaker_context_id);
}

// After a reconnect: pass the stored id back (valid for 24h).
recognizer.SetSpeakerDiarization(trtc_asr::v3::kSpeakerDiarizationCluster);
recognizer.SetEnableSpeakerContext(trtc_asr::v3::kSpeakerContextSync);
recognizer.SetSpeakerContextId(load("speaker_context_id"));
recognizer.Start(); // continue_status=resumed means the speaker anchors were restored
```
...
> **The v2 realtime API supports speaker context too**: query parameters `enable_speaker_context` (`1` sync / `2` async) and `speaker_context_id`, with the same `speaker_continue` on the first response. The C++ API is `SetEnableSpeakerContext` / `SetSpeakerContextId` / `GetSpeakerContinue()`. v2 synthesizes `OnRecognitionStart` locally, before the server's first response, so the handshake is only available from the getter. In sync mode with a stored id, wait until `GetSpeakerContinue()` has a value before `Write`: the server does not read audio until the snapshot has been applied.
...
CMake (`add_subdirectory` or `FetchContent`):
...
**Requires**: C++17, OpenSSL (TLS) and libcurl (HTTP). The WebSocket client and JSON layer are vendored.
...
// recognizer.Write(pcm.data(), pcm.size());   // loop over your audio
...
- Realtime (v3): [`examples/v3_realtime_asr.cc`](./examples/v3_realtime_asr.cc)
- Sentence (v3): [`examples/v3_sentence_asr.cc`](./examples/v3_sentence_asr.cc)
- Audio file (v3): [`examples/v3_file_asr.cc`](./examples/v3_file_asr.cc)
- Realtime (v2): [`examples/realtime_asr.cc`](./examples/realtime_asr.cc)
- Sentence (v2): [`examples/sentence_asr.cc`](./examples/sentence_asr.cc)
- Audio file (v2): [`examples/file_asr.cc`](./examples/file_asr.cc)

```bash
cmake -B build && cmake --build build -j
TRTC_ASR_SDK_APP_ID=... TRTC_ASR_SECRET_KEY=... ./build/v3_realtime_asr test.pcm bigmodel
```
...
│   └── v3.h                   # v3 protocol client (wire types + three clients)
├── src/
│   ├── *.cc                   # v2 implementation and internal helpers
│   └── v3.cc                  # v3 implementation
...
**v3 or v2?** For new integrations use v3 (`#include "trtc_asr/v3.h"`): only SdkAppID + SecretKey, a cleaner protocol, and `Start()` throws auth/parameter errors. Existing v2 users can stay as they are — see [docs/v2_protocol.md](./docs/v2_protocol.md).
...
**Where is the legacy v2 / v1 protocol?** The top-level `trtc_asr::` API stays maintained and is documented in [docs/v2_protocol.md](./docs/v2_protocol.md). v2 and v3 share the same downlink shape and listener API, so switching protocol versions only means changing the include and construction call.
